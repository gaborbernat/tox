...
def test_venv_redirect_prefers_editable(tox_project: ToxProjectCreator, demo_pkg_inline: Path) -> None:
    toml = 'env_list = [ "a", "b" ]\n[env.a]\npackage = "skip"\n[env.b]\npackage = "editable"\n'
    project = tox_project({"tox.toml": toml}, base=demo_pkg_inline, venv_redirect=True)
...
def test_tox_project_fixture_skips_venv_redirect_by_default(tox_project: ToxProjectCreator) -> None:
