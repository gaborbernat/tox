...
- The ``tox_project`` pytest fixture keeps the redirect off, so plugin test suites do not write ``.venv`` into their
  source tree; pass ``venv_redirect=True`` to test it.
